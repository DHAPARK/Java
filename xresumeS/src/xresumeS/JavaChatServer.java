...
public class JavaChatServer extends JFrame {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					try {
						System.out.println("오류가 나는 위치 탐색x");
						Thread.sleep(10000);
	...
	public JavaChatServer() {
		...
		lblNewLabel.setBounds(12, 10, 87, 26);
		...
		btnServerStart.setBounds(12, 12, 300, 35);
	...
	class UserService extends Thread {
		...
		public UserService(Socket client_socket) {
			...
			try {
				...
				if(UserVec.size() > 9) {
					...
					WriteAll(String.format("%s %s",UserName,"님이 입장하셨습니다."));
					WriteAll(String.format("%s %d","현재 참가자 수 ",UserVec.size()+1));
				}
				//이부분 참가자 수를위해 수정
		...
		public void WriteOne(String msg) {
			try {
				...
				try {
					//WriteAll(String.format("%s %s",UserName,"님이 퇴장하셨습니다"));
					dos.close();
					dis.close();
					client_socket.close();
					...
					if((msg.split(" ")[1]).toLowerCase().equals("/to")) {
						...
						while(iter.hasNext()) {
							...
							if(user.getUserName().equals(targetName) && user.isStatus() == false) {
								user.WriteOne(String.format("[귓속말] [%s] %s\n",UserName,msg.split(" ")[3]));
								break;
							}
						}
						continue;
